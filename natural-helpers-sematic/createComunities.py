...
def dataCleaning(data, title):
    ...
    with open(str(title) + ' .txt', 'w') as json_file:
        json_file.write(data)


def outputPropagatedComunity(generator, graph):
    """
    ...
    for x in comunity:
        ...
        if len(comunityData) > 1:
            dataCleaning(comunityData, 'Propagated Comunity' + str(count))
            count += 1
...
def makeFiles(directedGraph,undirectedGraph):
    ...
    outputPropagatedComunity(communityGenerator, G)
